...
def auto_number_preinstall_self_plan_prompt(data) -> str:

    info = (
        f"**Your Role & Mission:**\n"
        f"You are the designated **Decision Maker** agent for this Multi-Agent System (MAS). Your core responsibility is to receive the user's task, analyze it thoroughly, and decompose it into logical, actionable subtasks suitable for assignment to the worker agents.\n\n"
        f"**System Context:**\n"
        f"* **Worker Agents:** You need to decide how many work agent to allocate to this task yourself, identified by IDs such that 0,1,2,...\n"
    ...
    info += (
        ...
        f"**Your Objective & Required Output:**\n"
        f"1.  **Analyze:** Carefully consider the user task in light of the available agents, their communication links, and the tools they can use.\n"
        f"2.  **Decompose:** According to the number of work agents you decide on, the main user tasks are precisely decomposed into corresponding different subtasks. Ensure each subtask is well-defined and can realistically be assigned to a worker agent.\n"
        f"3.  **Provide Your Plan:** Output your analysis and the detailed decomposition plan below. Start with your reasoning and then list the subtasks.\n"
        f"**Your Analysis:**\n"
    )
    return info
...
class PlanningAgentInfo(AgentInfo):
    ...
    def __init__(
            ...
            data=None,
            self_deter=False,
            number_mode: str ='default'
    ) -> None:
        if number_mode == 'auto':
            self.info = auto_number_preinstall_self_plan_prompt(data)
        else:
            if self_deter: # True
