...
class Agent:
    ...
    async def _think(
        ...
        while True:
            ...
            print("------------------ Raw Result -----------------")
            print(f"Agent {self.name} raw_result: {raw_result}") # Debugging
    ...
    async def emulate_one_step(
        ...
        print(f"Agent {self.name} received message: {text_to_consider.prompt}...") # Debugging
        # sys.exit(0)
...
class PlanningAgent(Agent):
    ...
    def determine_topo(self):
        ...
        text_to_consider = (
            ...
            f"The list corresponding to neighborhood **CANNOT BE EMPTY**!!! At least one node included\n"
            f"**Output Specifications:**\n\n"
            f"* **Format:** Your response MUST be a valid JSON list (`[...]`).\n"
        ...
        while True:
            try:
                ...
                topo_list = json.loads(raw_topo)
                # print(f"Raw topology generated: {raw_topo}") # Debugging
                if any(len(item['neighborhood']) == 0 for item in topo_list):
                    print(f"Raw topology generated (invalid): {raw_topo}") # Debugging
                    repeat_time += 1
                    if repeat_time >= 5:
                        sys.exit(0)
                    continue
                final_topo = json.loads(raw_topo)
                print(f"Final Raw topology generated: {raw_topo}") # Debugging
                break
            except json.JSONDecodeError as e:
    ...
    def emulate_one_step(
        ...
        # print(f"Mid Plan: {mid_plan}")
        # sys.exit(0)
        print("================== Midplan Generated =================")
        # print(f"PlanningAgent {self.name} midplan: {mid_plan}")
        ...
        # print(f"PlanningAgent {self.name} assigned subtasks: {planning}")
        
        
        if self_deter:
            topo = self.determine_topo()
            print("================== Topology Determined =================")
            # print(f"PlanningAgent {self.name} determined topology: {topo}")
...
class DefenceAgent():
    ...
    def __init__(self, ground_truth = None):
        ...
        # print("Defence Agent using OpenAI Base URL:", openai_base_url)
        # print("Defence Agent using OpenAI API Key:", openai_api_key[:4] + "****" + openai_api_key[-4:])
        # # sys.exit(0)
